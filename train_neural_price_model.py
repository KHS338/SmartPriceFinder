...
try:
    ...
    print("[!] scikit-learn not properly installed")
...
def prepare_features(df):
    ...
    print("\n[i] Preparing features for Neural Network training...")
...
def train_model(X, y):
    """Train Neural Network model with 80-20 split."""
    ...
    print("\n[i] Training Neural Network (MLP) Regressor...")
    print("[i] Architecture: Input -> 128 -> 64 -> 32 -> Output")
...
def save_model(model, scaler, label_encoders, feature_cols, metrics):
    """Save model and encoders to disk."""
    os.makedirs(MODEL_DIR, exist_ok=True)
    
    model_path = os.path.join(MODEL_DIR, 'neural_price_model.pkl')
    ...
    model_package = {
        ...
        'description': 'Multi-Layer Perceptron neural network for price prediction'
    }
...
def main():
    ...
    print("="*60)
    print("Neural Network Price Prediction Model Training")
    print("Deep Learning with Multi-Layer Perceptron")
    print("="*60)
    ...
    # Train model
    model, scaler, X_test, y_test, metrics = train_model(X, y)
    
    # Save model
    model_path = save_model(model, scaler, label_encoders, feature_cols, metrics)
    
    print("\n" + "="*60)
    print("[OK] Training complete!")
    print(f"[OK] Model ready at: {model_path}")
    print(f"[OK] R² Score: {metrics['r2']:.4f}")
    print(f"[OK] MAE: {metrics['mae']:.2f} PKR")
    print(f"[OK] Accuracy (±10%): {metrics['acc_10']:.2f}%")
    print("="*60)
